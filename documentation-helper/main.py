    ...
    with st.chat_message("assistant:"):
        try:
            with st.spinner("Retrieving docs and generating answer…"):
                ...
                # This will catch "answer", "answer ", or "answer : " if you forget again!
                # Get the raw value (which we now know is a list)
                raw_val = result.get("answer") or result.get("answer : ")

                if isinstance(raw_val, list):
                    # If it's a list, join the elements into one string
                    answer = " ".join([str(i) for i in raw_val]).strip()
                elif raw_val:
                    answer = str(raw_val).strip()
                else:
